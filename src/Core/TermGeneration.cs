...
每项 text 为 1—64 字的单个词条，category 为简短的分类名称。相同词条不重复，禁止通过多余空格凑数；大小写有不同科学含义的单位、元素和化学式（如 mA/MA、Co/CO）分别保留。力求返回 requested_count 个，不够时按真实数量返回。
...
""";
    public static string Normalize(string text)=>Lexicon.WordKey(text);
    public static GeneratedTermBatch Parse(string json,TermGenerationOptions options,int limit)
    {
        ...
        var result=new List<TermData>();var seen=new HashSet<string>(StringComparer.Ordinal);int rejected=0;
