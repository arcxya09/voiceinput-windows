...
{
    public WholePolishWork? BeginWholePolish(bool enabled,string[] protectedTerms,bool smartPunctuation=true)
    {
        ...
        return new(Session.Id,Session.Generation,Session.WholePolishOperation,clock()+timeout,raw,protectedTerms){SmartPunctuationEnabled=smartPunctuation};
    }
    ...
    public bool CompleteWholePolish(WholePolishWork work,string? candidate,string? error=null)
    {
        ...
        try{check=candidate is null||clock()>=work.Deadline?new ValidationResult(false,work.Raw,error??"全文润色超时，保留原文"):PolishRules.Validate(work.Raw,candidate,work.ProtectedTerms,work.SmartPunctuationEnabled);}
