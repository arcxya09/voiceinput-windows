...
public partial class MainWindow
{
    ...
    private void RefreshLiveState()
    {
        ...
        UsageHelp.Text = $"1. 在设置中填写百炼 Key；需要全文润色时填写 DeepSeek Key。\n2. { (dictation ? "仅听写模式：可在任意窗口" : "在其他应用的文本框中") }按住 {key}，麦克风就绪后说话。松开即停止录音。\n3. { (dictation ? "整理完成后自动复制正文，可在目标输入框按 Ctrl+V。" : "整理完成后先复制，再整段粘贴到原位置，不自动按回车。没有可靠输入焦点或软件改变焦点时继续听写，完成后也会自动复制。") }\n4. 在“对照与编辑”中保存手动修订，到词库确认有用的纠错候选。\n5. Esc 取消本轮。松手等待上屏时，普通回车跳过剩余润色（不会转发回车发送消息）。其他按键或鼠标操作会改为完成后复制；上一轮仍在整理时请稍后再按。";
    ...
    private async Task SetDictationMode(bool enabled)
    {
        try
        {
            ...
            StatusText.Text = enabled ? "仅听写模式已开启：按住说话键，完成后自动复制正文。" : "自动粘贴已开启：请先选中其他应用的文本框，识别正文将替换剪贴板内容。";
        }
