...
public sealed partial class AppController
{
    ...
    private int EligibleHotwords()=>terms.Where(t=>t.State==TermState.Enabled&&(t.Scope=="*"||t.Scope==Settings.ProjectId))
        .Select(t=>TermGenerationRules.Normalize(t.Text)).Distinct(StringComparer.OrdinalIgnoreCase).Count();
    public async Task<string> LexiconReportAsync()
    {
        ...
        string state=s.HotwordState switch{"Sent"=>"已向 ASR 提交","Prepared"=>"已选取，尚未确认提交","Failed"=>"任务启动失败，未确认提交","Disabled"=>"本轮词库已关闭",_=>"旧记录无热词快照"};
