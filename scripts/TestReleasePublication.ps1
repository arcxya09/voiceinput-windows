...
. "$PSScriptRoot/PublishRelease.ps1" -Repository 'test/repo' -Commit ('a' * 40) -Version '2.0.0' -Notes $PSCommandPath -FunctionsOnly
$originals = @{}
...
foreach ($fixtureName in @('VoiceInput-Windows-x64-2.0.0.zip', 'VoiceInput-Source-2.0.0.zip', 'build-output.log', 'SHA256SUMS.txt')) {
...
function Set-PublicationFixture {
    $script:remote = [pscustomobject]@{ tag_name = 'v2.0.0'; target_commitish = ('a' * 40); draft = $true; assets = @() }
...
Test 'HTTP 502 is retried as a read and never classified as missing' {
    ...
    $failed = $false
    try { $null = Invoke-GitHubRead 'releases/tags/v2.0.0' -AllowMissing } catch { $failed = $true }
...
Test 'Only an authoritative HTTP 404 permits release creation' {
    ...
    Assert ($null -eq (Invoke-GitHubRead 'releases/tags/v2.0.0' -AllowMissing)) '404 was not classified as missing.'
    $failed = $false
    try { $null = Invoke-GitHubRead 'releases/tags/v2.0.0' } catch { $failed = $true }
...
Test 'Ambiguous creation is read back without creating a duplicate release' {
    ...
    Mock 'Invoke-GitHubCommand' {
        param($Arguments)
        $script:commands.Add(($Arguments -join ' '))
        if ($Arguments[1] -eq 'create') { $script:remote = [pscustomobject]@{ tag_name = 'v2.0.0'; target_commitish = ('a' * 40); draft = $true; assets = @() }; return 1 }
...
Test 'Published checksum manifest covers exact payloads and rejects traversal' {
    ...
    try {
        ...
        $names = @('VoiceInput-Windows-x64-2.0.0.zip', 'VoiceInput-Source-2.0.0.zip', 'build-output.log', 'SHA256SUMS.txt')
...
Test 'A draft without a tag ref is recovered through production listing and ID reads' {
    ...
    Mock 'Get-TagCommit' $originals['Get-TagCommit']
    $script:remote = [pscustomobject]@{ id = 123; tag_name = 'v2.0.0'; target_commitish = ('b' * 40); draft = $true; assets = @() }
...
Test 'Production draft discovery scans every list page and reads the matching ID' {
    ...
    Mock 'Invoke-GitHubRead' {
        param($Endpoint, [switch]$AllowMissing)
        if ($Endpoint -eq 'releases/tags/v2.0.0') { return $null }
        if ($Endpoint -eq 'releases?per_page=100&page=1') {
            $script:pages += 1
            return @(1..100 | ForEach-Object { @{ id = $_; tag_name = "older-$_"; draft = $false } })
        }
        if ($Endpoint -eq 'releases?per_page=100&page=2') { $script:pages += 2; return @(@{ id = 456; tag_name = 'v2.0.0'; draft = $true }) }
        if ($Endpoint -eq 'releases/456') { return @{ id = 456; tag_name = 'v2.0.0'; draft = $true; target_commitish = ('a' * 40) } }
        throw "Unexpected endpoint: $Endpoint"
    }
    $draft = Read-Release 'v2.0.0' -AllowMissing
...
Test 'Ambiguous creation waits for draft visibility without repeating creation' {
    ...
    Mock 'Invoke-GitHubRead' {
        param($Endpoint, [switch]$AllowMissing)
        if ($Endpoint -eq 'git/ref/heads/main') { return @{ object = @{ sha = ('a' * 40) } } }
        if ($Endpoint -eq 'git/ref/tags/v2.0.0') { if ($script:tagCreated) { return @{ object = @{ type = 'commit'; sha = ('a' * 40) } } }; return $null }
        if ($Endpoint -eq 'releases/tags/v2.0.0') { if ($script:remote -and !$script:remote.draft) { return $script:remote }; return $null }
        if ($Endpoint -eq 'releases?per_page=100&page=1') {
    ...
    Mock 'Invoke-GitHubCommand' {
        ...
        if ($Arguments[1] -eq 'create') {
            $script:remote = [pscustomobject]@{ id = 789; tag_name = 'v2.0.0'; target_commitish = ('a' * 40); draft = $true; assets = @() }
...
Test 'Duplicate tag matches or a changed release ID are rejected' {
    Mock 'Invoke-GitHubRead' {
        param($Endpoint, [switch]$AllowMissing)
        if ($Endpoint -eq 'releases/tags/v2.0.0') { return $null }
        return @(@{ id = 1; tag_name = 'v2.0.0' }, @{ id = 2; tag_name = 'v2.0.0' })
    }
    $failed = $false; try { $null = Read-Release 'v2.0.0' -AllowMissing } catch { $failed = $true }
    Assert $failed 'Ambiguous release identities must not be guessed.'
    Mock 'Invoke-GitHubRead' {
        param($Endpoint, [switch]$AllowMissing)
        if ($Endpoint -eq 'releases/tags/v2.0.0') { return $null }
        if ($Endpoint -eq 'releases?per_page=100&page=1') { return @(@{ id = 1; tag_name = 'v2.0.0' }) }
        return @{ id = 2; tag_name = 'v2.0.0' }
    }
    $failed = $false; try { $null = Read-Release 'v2.0.0' } catch { $failed = $true }
